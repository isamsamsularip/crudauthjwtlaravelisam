...
class ProductTest extends TestCase
{
    ...
    public function test_can_create_product()
    {
        ...
        $product = $productRepository->create([
            'title'       => 'DAIHATSU AYLA X MT',
            'price'       => 100,
            'user_id'     => 1,
            'description' => 'Bekas tahun 2014 body masih mulus kaki kaki empuk',
        ]);

        $this->assertDatabaseCount('products', $totalProducts + 1);

        // Delete the product as need to keep it in database for other tests
        $product = Product::where('title', 'DAIHATSU AYLA X MT')->where('price', 100)->first();
        $product->delete();
    }
}
